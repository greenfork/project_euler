defmodule CircularPrimes do
  def run do
    Enum.count(circular_primes())
  end

  def circular_primes do
    primes = Eratosthenes.primes()

    for {p, i} <- Enum.with_index(:array.to_list(primes)),
        p == true,
        circular_prime?(i, primes),
        do: i
  end

  def circular_prime?(n, primes), do: Enum.all?(rotations(n), fn x -> aget(primes, x) end)

  def rotate([a | b]), do: b ++ [a]

  def rotations(n) do
    digits = Integer.digits(n)
    [_ | rotations] = Enum.reduce(digits, [digits], fn _, [h | _] = acc -> [rotate(h) | acc] end)
    Enum.map(rotations, &Integer.undigits/1)
  end
...
# IO.inspect(CircularPrimes.circular_primes(), limit: :infinity, pretty: true)
IO.inspect(CircularPrimes.run())

# 197, 971, 719 - rotated numbers and all are primes
